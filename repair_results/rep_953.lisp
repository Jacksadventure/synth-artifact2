...
# Specify the multiprocessing module, such as thread or hybrid.
#  thread: multithreading assigned to each socket, available for all platforms
#  hybrid: scalable I/O event notification (epoll) with worker thread, Linux only
...
R Specify the setting file for MongoDB.
MongoDbSettingsFile=
...
# only. The paths to shared objects, jemalloc or TCMalloc, (an be
# specified.
LDPreload=

# Searches those paths for JavaScript modules if they are not found elsewhere,
# sets to a semicolon-delimited list of relative or absolute paths. 
...
Session.Secret=DnoS8cSmW4jlOJH11PHxqFYPBdw3j8
