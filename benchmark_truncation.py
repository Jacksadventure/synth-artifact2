...
def load_mutations(max_orig: int, dbs: dict):
    for fmt, db in dbs.items():
        if not os.path.exists(db):
            print(f"[WARN] DB {db} missing for format '{fmt}'")
            continue
...
def insert_samples(samples, db_path: str):
...
def validate(path: str, fmt: str) -> bool:
    ...
    try:
        res = subprocess.run(
            [exe, path], stdout=subprocess.PIPE, stderr=subprocess.PIPE,
...
def oracle_info(out: bytes):
    ...
    m = re.search(
        r"oracle runs: (\d+) correct: (\d+) incorrect: (\d+) incomplete: (\d+)", txt
    )
    return tuple(map(int, m.groups())) if m else (0, 0, 0, 0)


def repair_and_update(cur, conn, row, idx: int, total: int):
    ...
    cur.execute("""UPDATE results SET
                   repaired_text=?, fixed=?, iterations=?, repair_time=?,
                   correct_runs=?, incorrect_runs=?, incomplete_runs=?,
...
def main():
    parser = argparse.ArgumentParser(
        description="Run repairs on mutated inputs with optional formats and DB path"
    )
    parser.add_argument(
        '-f', '--formats', nargs='+', choices=list(ALL_MUTATION_DBS.keys()),
        default=['dot'], help="Formats to process (default: ['dot'])"
    )
    parser.add_argument(
        '-o', '--result-db', default=DEFAULT_RESULT_DB,
        help="Path to the results database (default: result12.db)"
    )
